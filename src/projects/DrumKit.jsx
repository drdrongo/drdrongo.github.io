...
const DrumKit = () => {
  ...
  const addAnimations = useCallback(() => {
    Object.values(drums).forEach(({ id, key, audio }) => {
      ...
      const handleKeyDown = e => onKeyDown({ e, key, drum, audio });
      const handleKeyUp = e => onKeyUp({ e, key, drum });
      drumListeners[`${id}-down`] = handleKeyDown;
      document.addEventListener('keydown', handleKeyDown);
      drumListeners[`${id}-up`] = handleKeyUp;
  ...
  useEffect(() => {
    addAnimations();
    return () => {
      Object.keys(drumListeners).forEach(key => {
      ...
      <div className="sequencer-container">
        ...
        <div className="sequencer-options">
          ...
          <button onClick={onPlayClick} className="play-button">
            <img className="play-button-img" src={playing ? imgPause : imgPlay} alt="playing or pausing" />
          </button>
